...
DEFAULT_INPUT_DIR="/mnt/aigc/houyuanchen/pinterest-dl/light_repo/pin_996421486310676075_related"
DEFAULT_SYSTEM_PROMPT="$VLM_DIR/prompts/sheer_curtain_light.zh.md"
DEFAULT_API_CONFIG="$VLM_DIR/api/gpt_5_6_luna.json"
DEFAULT_OUTPUT_DIR="$VLM_DIR/output/gpt_5_6_luna_pin_996421486310676075_related_sheer_curtain_light"
DEFAULT_USER_PROMPT="请严格检查这张图片是否符合大面积轻柔纱窗帘主体、窗帘上具有明显光影并且整体具有较强美感的要求，并只按系统要求返回一个 JSON 对象。"
...
run_worker() {
    ...
    if [[ -s "$result_path" ]] && jq -e '
        (.sheer_curtain_subject | type == "boolean")
        and (.obvious_curtain_light_shadow | type == "boolean")
        and (.aesthetically_pleasing | type == "boolean")
    ...
    if python3 "$CLASSIFIER" \
        ...
        if jq --arg image "$(realpath "$image_path")" '
            if (
                ...
                and (.reasons.subject | type == "string" and length > 0)
                and (.reasons.light_shadow | type == "string" and length > 0)
                and (.reasons.aesthetics | type == "string" and length > 0)
            ) then
                .image = $image
                | .all_pass = (
                    .sheer_curtain_subject
                    and .obvious_curtain_light_shadow
                    and .aesthetically_pleasing
                )
            else
                error("model result does not match the required sheer curtain light schema")
            end
