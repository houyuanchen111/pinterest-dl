...
DEFAULT_INPUT_DIR="$VLM_DIR/output/gpt_5_6_luna_lush_forest/passed"
DEFAULT_API_CONFIG="$VLM_DIR/api/gpt_5_6_luna.json"
DEFAULT_OUTPUT_DIR="$VLM_DIR/output/gpt_5_6_luna_lush_forest_light_score"

validate_score_result() {
    local result_path="$1"
    jq -e '
        .light_visibility_score as $visibility
        | .light_shadow_layering_score as $layering
        | .light_aesthetic_score as $aesthetic
        | .light_shadow_score as $total
        | (($visibility | type) == "number")
        and (($visibility | floor) == $visibility)
        and ($visibility >= 0 and $visibility <= 40)
        and (($layering | type) == "number")
        and (($layering | floor) == $layering)
        and ($layering >= 0 and $layering <= 30)
        and (($aesthetic | type) == "number")
        and (($aesthetic | floor) == $aesthetic)
        and ($aesthetic >= 0 and $aesthetic <= 30)
        and (($total | type) == "number")
        and (($total | floor) == $total)
        and ($total >= 0 and $total <= 100)
        and ($total == ($visibility + $layering + $aesthetic))
...
run_worker() {
    ...
    if [[ -s "$result_path" ]] \
        && jq -e . "$result_path" >/dev/null 2>&1 \
        && validate_score_result "$result_path"; then
        printf '%s\tSKIP\t%s\n' "$(date '+%F %T')" "$image_path" >>"$events_log"
        return 0
    fi

    work_dir="$(mktemp -d "${TMPDIR:-/tmp}/lush_forest_light_score.XXXXXX")"
    temp_result="$work_dir/result.json"
    temp_error="$work_dir/error.log"
    normalized_result="$work_dir/normalized.json"
    ...
    if [[ "$image_suffix" != "jpg" && "$image_suffix" != "jpeg" && "$image_suffix" != "png" ]] \
        ...
        preprocessed_image="$work_dir/preprocessed.jpg"
...
PY
    ...
    for ((attempt = 1; attempt <= retries; attempt++)); do
        ...
        if python3 "$CLASSIFIER" \
            "$classifier_image" \
            --prompt lush_forest_light_score \
            ...
            if jq --arg image "$(realpath "$image_path")" '.image = $image' \
                ...
                && [[ -s "$normalized_result" ]] \
                && jq -e . "$normalized_result" >/dev/null 2>&1 \
                && validate_score_result "$normalized_result"; then
                cp -f -- "$normalized_result" "$result_path"
                if [[ -s "$result_path" ]] \
                    && jq -e . "$result_path" >/dev/null 2>&1 \
                    && validate_score_result "$result_path"; then
                    rm -f "$error_path"
            ...
            printf 'Model result failed score validation on attempt %s.\n' "$attempt" \
                >>"$temp_error"
        fi
...
mkdir -p "$OUTPUT_DIR/results" "$OUTPUT_DIR/errors"
...
RAW_SUMMARY="$OUTPUT_DIR/summary.unsorted.jsonl"
find "$OUTPUT_DIR/results" -maxdepth 1 -type f -name '*.json' -size +0c -print0 \
    | sort -z \
    | xargs -0 -r -n 1 jq -c . \
    >"$RAW_SUMMARY"
jq -s -c 'sort_by(.light_shadow_score) | reverse | .[]' \
    "$RAW_SUMMARY" >"$OUTPUT_DIR/summary.jsonl"
head -n 200 "$OUTPUT_DIR/summary.jsonl" >"$OUTPUT_DIR/top_200.jsonl"
rm -f "$RAW_SUMMARY"
