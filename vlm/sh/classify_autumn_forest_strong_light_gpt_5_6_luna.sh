...
DEFAULT_INPUT_DIR="/mnt/aigc/houyuanchen/pinterest-dl/light_repo/visual_keywords/autumn_forest_merged"
DEFAULT_SYSTEM_PROMPT="$VLM_DIR/prompts/autumn_forest_strong_light.zh.md"
DEFAULT_API_CONFIG="$VLM_DIR/api/gpt_5_6_luna.json"
DEFAULT_OUTPUT_DIR="$VLM_DIR/output/gpt_5_6_luna_autumn_forest_merged_strong_light"
DEFAULT_USER_PROMPT="请严格检查这张图片是否符合秋日暖色树林、极其明显且优美的光影、真实摄影风格和极高审美要求，并只按系统要求返回一个 JSON 对象。"
...
run_worker() {
    ...
    if [[ -s "$result_path" ]] && jq -e '
        (.autumn_forest_subject | type == "boolean")
        and (.warm_overall_tone | type == "boolean")
        and (.extremely_prominent_light_shadow | type == "boolean")
        and (.photorealistic_style | type == "boolean")
        and (.exceptional_light_aesthetics | type == "boolean")
        and (.all_pass | type == "boolean")
...
PY
    ...
    if python3 "$CLASSIFIER" \
        ...
        --user-prompt "$DEFAULT_USER_PROMPT" \
        --api-config "$api_config" \
        --timeout "$timeout" \
        --retries "$retries" \
        --output "$temp_result" \
        >/dev/null 2>"$temp_error"; then
        if jq --arg image "$(realpath "$image_path")" '
            if (
                (.autumn_forest_subject | type == "boolean")
                and (.warm_overall_tone | type == "boolean")
                and (.extremely_prominent_light_shadow | type == "boolean")
                ...
                and (.reasons.autumn_forest | type == "string" and length > 0)
                and (.reasons.warm_tone | type == "string" and length > 0)
                and (.reasons.light_shadow | type == "string" and length > 0)
                and (.reasons.photorealism | type == "string" and length > 0)
                and (.reasons.light_aesthetics | type == "string" and length > 0)
            ) then
                .image = $image
                | .all_pass = (
                    .autumn_forest_subject
                    and .warm_overall_tone
                    and .extremely_prominent_light_shadow
                    and .photorealistic_style
                    and .exceptional_light_aesthetics
                )
            else
                error("model result does not match the required autumn forest schema")
            end
        ' "$temp_result" >"$normalized_result" 2>>"$temp_error" \
            && [[ -s "$normalized_result" ]] \
...
xargs -0 -r -P "$CONCURRENCY" -I '{}' \
    "$SCRIPT_PATH" --worker \
    '{}' "$OUTPUT_DIR" "$SYSTEM_PROMPT" "$API_CONFIG" "$TIMEOUT" "$RETRIES" \
    <"$IMAGE_LIST" &
XARGS_PID=$!
